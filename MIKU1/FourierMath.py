...
import os
'''
ADAPTED FROM 中梓星音https://github.com/ruanluyu/FourierCircleDrawing
'''
INPUTFILE = 'MIKUC.txt'
OUTPUTFILE = 'MIKUC_datas.txt'
start = 0
end = 2000#圆圈数量
...
def mainCalculation(s):
    ...
    with prjCur.get_lock():
        prjCur.value += 1
    return cpToList(3*sum)

if __name__ == '__main__':
    with open(INPUTFILE, 'r') as f:#读取并解析SVG信息
        rawdata = f.readlines()
    ...
    with open(OUTPUTFILE, "w") as File:#把结果写入硬盘
        for dc in out:
